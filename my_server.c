...
void http_default(struct evhttp_request *req, void *arg)
{
    ...
    //3 get header
    struct evkeyvalq *headers = evhttp_request_get_input_headers(req);//消息队列
    for (struct evkeyval *p = headers->tqh_first; p!=NULL; p = p->next.tqe_next)
    {
...
static void
post_method (struct evhttp_request *req, void *arg)
{ 
    ...
    //判断请求类型
    if (evhttp_request_get_command (req) == EVHTTP_REQ_GET)/* 判断 req 是否是GET 请求 */
    {
    ...
    if (evhttp_request_get_command (req) != EVHTTP_REQ_POST)/* 这里只处理Post请求, Get请求，就直接return 200 OK  */
    { 
        evhttp_send_reply (req, 200, "OK", NULL);
        return;
    }
...
static void
get_method (struct evhttp_request *req, void *arg)
{
...
{ 
    ...
    */
    ...
    // TODO: 这里释放连接。如果要实现单个连接多个req，是否考虑在这里判断超时时间？
    if (decoded)
        evhttp_uri_free (decoded);
    if (evb)
        evbuffer_free (evb);
...
{ 
    ...
    if (1 != SSL_CTX_use_certificate_chain_file (ctx, certificate_chain))   //TODO: 这里证书会提示异常：证书过小。怀疑是因为每一个证书长度是1024的。但是怎么生成2048？
...
{ 
    ...
    struct evhttp *http; //TODO: 一个http对应多个connection；每个connection对应多个req/ or http上下文 / or建立连接
    ...
    */
    ...
    // evhttp_set_cb(http, "/rest/atas/sessionBegin.do", Object, NULL); // TODO: ‘Object’ undeclared (first use in this function) 检查一下1 源仓库中是否有这个问题 2 看看别的编译方式
    ...
    //self-modeled method
    evhttp_set_cb(http, "/post_method", post_method, NULL);
    evhttp_set_gencb(http, http_default, NULL);

    // evconnlistener_new_bind() //创建、bind、listen、accept socket
    /* 设置监听IP和端口 */
    ...
    // 实现ipv6(?)
    {
        ...
        //AF_INET6适用于Internet IPv6 和 IPv4 系列
